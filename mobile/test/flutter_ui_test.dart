...
void main() {
  ...
  group('空账本 UI', () {
    ...
    testWidgets('一句话记账经 Parser 写入真实 SQLite 并刷新统计', (tester) async {
      ...
      await tester.tap(find.byKey(const Key('quick-submit')));
  ...
  group('单条真实账目 UI', () {
    ...
    testWidgets('删除使用软删除，撤销后恢复并联动统计', (tester) async {
      ...
      await _confirmDeleteAndWait(tester, fixture.controller);
...
Future<void> _openDraft(WidgetTester tester, String text) async {
  await tester.enterText(find.byKey(const Key('quick-input')), text);
  await tester.tap(find.byKey(const Key('quick-submit')));
  await tester.pumpAndSettle();
}
...
Future<void> _confirmDeleteAndWait(
  ...
  await tester.pump();
  await tester.runAsync(() => _waitForControllerIdleInRealAsync(controller));
  await tester.pumpAndSettle();
}
